...
function activate(context) {
    ...
    const updateStatusBar = async () => {
        ...
        try {
            const command = config.get('command', 'npx ccusage@latest blocks');
            ...
            if (usageData) {
                const showCurrency = config.get('showCurrency', 'USD');
                const jpyRate = config.get('jpyRate', 150);
                const showTimeRemaining = config.get('showTimeRemaining', true);
                let statusText = '';
                // Plan indicator
                if (usageData.isMax) {
                    statusText = '$(zap) MAX ';
    ...
    const startUpdateInterval = () => {
        const config = vscode.workspace.getConfiguration('claudeCodeStatus');
        const interval = config.get('interval', 5000); // 5秒間隔に短縮
        if (updateInterval) {
    ...
    context.subscriptions.push(statusBarItem, refreshCommand, showDetailsCommand, showConfigCommand, configWatcher);
    if (updateInterval) {
...
function createTooltip(data, currency, jpyRate) {
...
function calculateSessionEnd(startTimeJST, remainingTime) {
    try {
        // Parse start time
